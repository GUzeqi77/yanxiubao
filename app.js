...
function bindTrainingHeatmap() {
  ...
  const provinceFilter = document.querySelector("#provinceProjectFilter");
  const projectList = document.querySelector("#provinceProjectList");
  const projectSummary = document.querySelector("#provinceProjectSummary");
  ...
  let dragStart = null;
  let activeDatasetLabel = "全部";
  let activeProvince = "全国";
  ...
  const getProvinceProjects = () => {
    const dataset = heatmapTrainingData[activeDatasetLabel] || heatmapTrainingData["全部"];
    const entries = activeProvince === "全国"
      ? Object.entries(dataset).sort((a, b) => b[1] - a[1]).slice(0, 8)
      : [[activeProvince, dataset[activeProvince] || 0]];

    return entries.flatMap(([province, value], provinceIndex) => {
      if (!value) return [];
      const visibleCount = activeProvince === "全国" ? 1 : 4;
      return Array.from({ length: visibleCount }, (_, index) => {
        const ratio = [0.24, 0.18, 0.14, 0.1][index] || 0.1;
        return {
          province,
          title: `${province}${activeDatasetLabel === "全部" ? "" : activeDatasetLabel}${projectNames[index % projectNames.length]}`,
          org: provinceProjectOrgs[province] || `${province}教师发展中心`,
          count: Math.max(68, Math.round(value * ratio)),
          year: activeDatasetLabel === "全部" ? ["2026年", "2025年", "2024年"][provinceIndex % 3] : activeDatasetLabel,
          type: ["混合研修", "专题培训", "校本实践", "成果展示"][index % 4]
        };
      });
    });
  };

  const renderProvinceProjects = () => {
    if (!projectList) return;
    const projects = getProvinceProjects();
    const label = activeDatasetLabel === "全部" ? "全部年份" : activeDatasetLabel;
    if (projectSummary) {
      projectSummary.textContent = `${activeProvince} · ${label} · ${projects.length} 个项目`;
    }
    projectList.innerHTML = projects.length
      ? projects
          .map((project) => `
            <article class="province-project-card">
              <div class="province-project-meta">
              ...
              <div class="province-project-meta">
                <em>${project.type}</em>
                <strong>${formatNumber(project.count)}人</strong>
              </div>
            </article>
          `)
          .join("")
      : `<div class="province-project-empty">暂无${activeProvince}${label}项目</div>`;
  };

  const setActiveProvince = (province) => {
    activeProvince = province || "全国";
    if (provinceFilter) provinceFilter.value = activeProvince;
    paths.forEach((path) => {
      path.classList.toggle("active", activeProvince !== "全国" && path.dataset.province === activeProvince);
    });
    renderProvinceProjects();
  };
  ...
  paths.forEach((path) => {
    ...
    path.addEventListener("click", () => setActiveProvince(path.dataset.province || "全国"));
  });
  ...
  if (provinceFilter) {
    const provinceOptions = ["全国", ...Object.entries(heatmapTrainingData["全部"]).sort((a, b) => b[1] - a[1]).map(([name]) => name)];
    provinceFilter.innerHTML = provinceOptions.map((name) => `<option value="${name}">${name}</option>`).join("");
    provinceFilter.value = activeProvince;
    provinceFilter.addEventListener("change", () => setActiveProvince(provinceFilter.value));
  }
...
function bindAiAssistant() {
  ...
  const toggle = document.querySelector(".ai-float-button");
  const panel = document.querySelector(".ai-panel");
  const close = document.querySelector(".ai-close");
  const chat = document.querySelector(".ai-chat");
  const form = document.querySelector(".ai-input");
  const input = form?.querySelector("input");
  const quickButtons = document.querySelectorAll("[data-ai-question]");
  if (!assistant || !toggle || !panel || !chat || !form || !input) return;
  ...
  const addMessage = (type, text) => {
    ...
    message.innerHTML = `<span>${type === "user" ? "我" : "研修宝"}</span><p>${text}</p>`;
    chat.append(message);
  ...
  const getAnswer = (question) => {
    ...
    if (/实施|方案|计划|怎么开展/.test(text)) {
      return "<strong>实施方案</strong>建议按“诊断测评-专家导学-工具实操-课堂实践-成果展示”推进。专题重点覆盖AI认知、AI工具应用、AI教学设计、AI课堂融合、AI教研创新和AI成果产出六个阶段，可结合校本教研组织线上学习、任务实践和阶段复盘。";
    }
    if (/考核|评价|结业|作业|学时|成绩/.test(text)) {
      return "<strong>考核内容</strong>通常由学习进度、互动参与、实践作业和成果提交组成。建议重点准备一份AI辅助教学设计、一段课堂应用反思或案例说明，并按培训批次通知完成结业考核材料提交。";
    }
    if (/安排|最近|时间|日程|报名|研修/.test(text)) {
      return "<strong>最近研修安排</strong>可关注新闻通知中的三项：5月18日春季AI融合教学专题培训结业考核安排，4月28日暑期AI教师发展训练营报名开放，以及后续第二批校本研修成果展示材料提交提醒。";
    }
    if (/路径|学习|开始|怎么学|阶段|能力/.test(text)) {
      return "<strong>学习路径</strong>建议先完成AI认知与工具应用，再进入AI教学设计和课堂融合，最后沉淀教研创新与成果案例。新手可先从备课、资源生成、课堂提问、作业反馈四个高频场景切入。";
    }
    if (/成果|提交|案例|作品|展示/.test(text)) {
      return "<strong>成果提交</strong>可以围绕教学设计、课件资源、课堂案例、教学反思四类整理。建议写清楚应用场景、AI工具使用方式、课堂变化、学生反馈和可复制经验，便于专家评价与区域展示。";
    }
    if (/专家|老师|团队|指导/.test(text)) {
      return "<strong>专家支持</strong>包含教育技术、跨学科课堂、区域教研和AI产品应用等方向，可为教学设计打磨、课堂案例诊断、成果凝练和校本研修组织提供指导。";
    }
    return "我可以围绕本专题继续帮您梳理。您可以问“实施方案怎么安排”“考核需要提交什么”“最近研修日程是什么”或“新教师应该先学哪一阶段”。";
  };
  ...
  const ask = (question) => {
    ...
    const thinking = addMessage("bot", "正在检索专题信息...");
    window.setTimeout(() => {
      thinking.querySelector("p").innerHTML = getAnswer(value);
      chat.scrollTop = chat.scrollHeight;
    }, 420);
  };
